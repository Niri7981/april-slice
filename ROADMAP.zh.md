...
- [x] 阶段 1：早晨到夜晚的本地世界闭环已经跑通。
- [x] Initial Hand 的 LLM 生成链路已经接上 Worker 和 Zod schema。
- [x] Echo 的 LLM resolver 代码链路已经接通：prompt、Worker `/echo`、schema、toneFilter、fallback、本地结算。
- [x] 2D Pixi 渲染层已经移除，现在只保留 Three.js 3D 版本。
- [x] 3D 白盒已经可跑：胶囊玩家、胶囊 April、地面、场景标记、Note Echo 物件。
- [x] 3D 控制已经改成游戏式控制：`WASD` 控 orbit camera，方向键按当前镜头方向移动角色。
- [ ] 真实上游模型稳定性还没完成验收。
- [ ] `fakeResolver` 和 `llmResolver` 的同场景对照还没完成验收。
- [ ] `docs/tone-rejects.md` 还没开始沉淀坏输出。
- [ ] 正式 3D 建模、美术、动画、声音还没开始。
...
原因很简单：现在 3D 已经成为唯一渲染方向，建模前必须先让镜头、移动、比例和空间感站住。
...
目标：先把 3D 白盒做到能承载情绪，再替换正式模型、动画、声音。

要做：

- April 和玩家的 3D 模型与基础动画。
- 场景模型替换占位几何。
- 夜晚版本。
- 脚步声、远处列车、风声、纸张声。
- 开头十分钟体验重做。
...
## 阶段 6：Three.js 切换（已提前启动）

原计划是前面都过了才进入 3D；现在项目已经提前切到 3D，并移除了 2D Pixi 路线。
...
```text
在当前 3D 白盒里确认建模前的尺度、镜头和移动手感，
然后开始替换玩家 / April / 场景占位模型。
```

不要先做复杂系统，也不要继续抽象 renderer。

我们现在要验证的是：

```text
这个空间是不是已经值得被建模，
玩家绕着 April 走、看、停下来的时候，有没有“在一个地方”的感觉。
```
